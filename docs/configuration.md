...
| `context_tokens` | `32768` | How large a prompt one request may hold. Raise it for a model that has to read whole files; lower it on a machine short of memory. The cache for a context is larger than the weights, so this costs more memory than the model does. |
