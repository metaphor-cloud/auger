"""Ask the model which of Semgrep's matches are real.

Semgrep matches a pattern. It does not read what surrounds the match, so a large share
of what it reports is already handled, already unreachable, or the intended use. This
pass sends the rule, the place, and the code there, and asks whether the problem is real
in this code.

Only the matched code goes to the model, never the whole repository, which is why a
scan costs a fraction of a review.

A verdict of "false" hides the finding from the list. That is a local model overruling a
security scanner, so the prompt tells it to answer "uncertain" rather than guess.
"""
