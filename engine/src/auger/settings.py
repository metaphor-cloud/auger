...
from auger.log import Level

# The webview is a different origin from the engine, so the browser sends a preflight
# before every authorised request. These are the origins a auger webview can have:
...
class Settings:
    ...
    def from_env(cls) -> Settings:
        level = os.environ.get("AUGER_LOG_LEVEL", "info")
        if level not in ("debug", "info", "warn", "error"):
            level = "info"
        return cls(
            host=os.environ.get("AUGER_HOST", "127.0.0.1"),
            ...
            port=int(os.environ.get("AUGER_PORT", "0")),
            # A generated token keeps a developer run usable with no host process.
            token=os.environ.get("AUGER_TOKEN") or secrets.token_urlsafe(32),
    ...
    def _origins() -> tuple[str, ...]:
        raw = os.environ.get("AUGER_ALLOWED_ORIGINS")
        if not raw:
