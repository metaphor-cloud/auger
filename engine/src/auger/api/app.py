...
from auger.store.index import chunk_count
from auger.store.notes import add_note, notes_for
from auger.store.runs import list_runs
from auger.store.summary import summarise
from auger.tracker import PERSON_SOURCE
...
def _dashboard(rig: Rig) -> DashboardOut:
    ...
    from auger.config import is_excluded
...
def create_app(rig: Rig) -> FastAPI:
    ...
    async def setup_models(request: SetupRequest) -> SetupOut:
        ...
        result = await rig.setup_models(
            request.model or None, request.embed or None, request.adversary or None
        )
        return SetupOut(
