...
from reviewrig.sandbox.base import Network, RunResult, RunSpec, SandboxError
...
class Seatbelt:
    ...
    def run(self, spec: RunSpec) -> RunResult:
        ...
        with tempfile.TemporaryDirectory(prefix="reviewrig-scratch-") as scratch:
            scratch_path = Path(scratch).resolve()
            profile = self.profile(scratch_path)
            environment = {
                **spec.env,
                "REVIEWRIG_WORK": str(spec.repository),
                "REVIEWRIG_SCRATCH": str(scratch_path),
